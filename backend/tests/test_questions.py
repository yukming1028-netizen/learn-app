...
def test_get_next_question(client, child_id, db_session):
    ...
    resp = client.post("/api/questions/next", json={"child_id": child_id, "subject": "math"})
    assert resp.status_code == 200
    data = resp.json()
    assert data is not None
    assert data["content"] == "3+4=?"
    assert "answer" not in data  # answer not exposed
...
def test_submit_answer_wrong(client, child_id, db_session):
    q = Question(subject="math", grade=1, difficulty=1, type="choice",
