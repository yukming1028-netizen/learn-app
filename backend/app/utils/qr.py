"""QR token generation and verification for device binding."""
import base64
import hashlib
import hmac
import json
...
def generate_device_bind_token(device_uuid: str) -> tuple[str, datetime]:
    """Generate a signed device binding token.
...
def verify_device_bind_token(token: str) -> Optional[dict]:
    """Verify a device binding token. Returns payload dict or None if invalid/expired."""
    try:
